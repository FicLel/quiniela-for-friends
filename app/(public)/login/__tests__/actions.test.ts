...
jest.mock('next/navigation', () => ({
  redirect: (url: string) => {
    mockRedirect(url)
    // Simulate the Next.js internal redirect throw so callers can't catch it.
...
describe('createFirstAdmin server action', () => {
  ...
  it('returns UNKNOWN_ERROR when email is invalid', async () => {
    const result = await createFirstAdmin('not-an-email', 'Password1!', 'Password1!')
  ...
  it('returns UNKNOWN_ERROR when password is too short', async () => {
    const result = await createFirstAdmin('admin@example.com', 'short', 'short')
    expect(result).toEqual({ success: false, error: 'UNKNOWN_ERROR' })
    expect(mockCreateFirstAdmin).not.toHaveBeenCalled()
  })

  it('returns UNKNOWN_ERROR when passwords do not match', async () => {
    const result = await createFirstAdmin('admin@example.com', 'Password1!', 'Password2!')
    expect(result).toEqual({ success: false, error: 'UNKNOWN_ERROR' })
    expect(mockCreateFirstAdmin).not.toHaveBeenCalled()
  })
  ...
  it('propagates SETUP_ALREADY_COMPLETE from AuthService', async () => {
    ...
    const result = await createFirstAdmin('admin@example.com', 'Password1!', 'Password1!')
  ...
  it('sets session cookie and redirects to /auth/change-password on success', async () => {
    mockCreateFirstAdmin.mockResolvedValueOnce({ success: true, token: 'jwt-token' })

    await expect(
      createFirstAdmin('admin@example.com', 'Password1!', 'Password1!'),
    ).rejects.toThrow('NEXT_REDIRECT:/auth/change-password')

    expect(mockSetSessionCookieOnServerAction).toHaveBeenCalledWith('jwt-token')
    expect(mockRedirect).toHaveBeenCalledWith('/auth/change-password')
  })

  it('normalises email to lowercase before calling AuthService', async () => {
    mockCreateFirstAdmin.mockResolvedValueOnce({ success: false, error: 'UNKNOWN_ERROR' })

    await createFirstAdmin('ADMIN@EXAMPLE.COM', 'Password1!', 'Password1!')
